...
その理由として、メタゲノム情報は細菌の種類や構成比を明らかにするが、実際の代謝活性は考慮されておらず、死菌のDNAも含まれるため、最終的な機能的出力を必ずしも反映しないことが挙げられる。
...
特定の_bifidobacterum_属の菌はHMOsを分解・利用する能力を有しており、HMOsはこれら有益菌の増殖を優先的に促進するプレバイオティクスとして機能する @borewiczEffectPrebioticFortified2019 。
...
抽出後、13,000 rpm、4 $degree$C で5分間遠心分離し、約 1.5 mL の上清を回収して -80 $degree$C で保存した。
...
主成分分析（PCA）の結果、スコアプロット（@fig-bmManual A）において、5名の解析者が同一の試料セット（n=4）を独立して解析した結果、各データ点は解析者によらず近接してプロットされており、クラスターの形成が確認できた。そのためPCAによる評価では、解析者間で顕著な差異がないことが示された。
...
== 母乳の#super[1]H-NMR測定および代謝物同定・定量<sec5.1>
...
母乳中メタボロームの経時的な変化を俯瞰的に評価するため、代謝物の濃度定量値に基づいたPCAを実施した（@fig-bmPCA）。採取時期別に色分けしたスコアプロットの結果、初乳（Colostrum）、産後1ヶ月（Aft. 1 mo.)、および産後4–5ヶ月（Aft. 4-5 mos.）の各群において、明確なクラスターの形成が認められた。

第1主成分（PC1）および第2主成分（PC2）は、主に初乳とそれ以外の群（産後1ヶ月・4–5ヶ月）の分離に寄与していた。ローディングプロットの解析から、2'-fucosyllactose (2'-FL)、3'-sialyllactose (3'-SL)、6'-sialyllactose (6'-SL) といったヒトミルクオリゴ糖（HMOs）や _myo_-inositol が、初乳群に多量に含まれる代謝物であることが示された。対して、産後1ヶ月および4–5ヶ月群では、主要なエネルギー源である lactose や、免疫系に関与するとされるbutyrate、caprate、caprylateなどの脂肪酸が多く含まれていた。

また、第3主成分（PC3）は、産後1ヶ月群と4–5ヶ月群の分離に寄与していた。産後1ヶ月群では、HMOsの一種である 3-fucosyllactose (3-FL) や、ともにTCA回路に関与するlactate、pyruvate などの有機酸が相対的に高値を示した。一方、産後4–5ヶ月群ではミネラル吸収を促進させる働きがある citrate や、 細胞膜の主要構成脂質であるphosphatidylcholineの前駆体である_O_-phosphocholine が多量に検出された。


== 分泌型/非分泌型における母乳中メタボロームの差異
@sec5.3 におけるPCAの結果、2'-FLの濃度変動が顕著であった。そこで、2'-FLに注目した詳細な解析を行うため、分泌型（Se+）および非分泌型（Se-）を目的変数とした判別分析（OPLS-DA）を実施した（@fig-bmOPLSDA）。
Se+/Se-の分類は、2'-FL濃度 0.1 mM を閾値として行った。
...
== 母子便中メタボロームのPCA分析
母子それぞれの便中メタボロームの時期的な変遷を俯瞰的に評価するため、NMRスペクトルのビン積分値データを用いたPCAを実施した（@fig-fePCA）。

乳児の便試料において、採取時期別に色分けしたスコアプロットを作成したところ、成長に伴う明確なクラスターの形成が認められた。具体的には、PC1の寄与により、日齢の低い「出産直後（Newborn）」群と、離乳が進んだ「産後1.5年（Aft. 1.5 yrs.）」、「産後3年（Aft. 3 yrs.）」、「産後5年（Aft. 5 yrs.）」の各群が、それぞれクラスターを形成して分布していた。
そして、その間を「産後1ヶ月（Aft. 1 mo.）」、「産後4-5ヶ月（Aft. 4-5 mos.）」、「産後8ヶ月（Aft. 8 mos.）」の群が経時的に変化するような分布を示していた。
一方、母親便においては、乳児で見られたような産後時期に伴う明確なクラスター形成や、経時的なメタボローム構成の変化は認められなかった。また、母子いずれの試料においても、母体の2'-FL 分泌型/非分泌型の差異に基づく明確な分離は確認されなかった。

== 母子便中メタボロームの産後時期によるOPLS-R回帰分析
...
乳児便においては、@fig-feOPLSR A に示す通り、適合度および予測精度の高い回帰モデルを構築することができた（$R^2Y=0.907$, $Q^2=0.866$）。S-lineプロット（@fig-feOPLSR B）を用いた解析の結果、エネルギー源であるglucose、lactose、galactose などの糖類や _myo_-inositol が、産後早期の便を特徴付ける主要な代謝物であることが示された。
母親の便中メタボロームについても、試料の採取時期（妊娠24週（24 wks. pregnant）、出産直後（Just after birth）、生後1ヶ月、生後4-5ヶ月）に対し、順序変数として1から4までの整数値を割り当て、これらを目的変数とした回帰モデルを構築した（@fig-feOPLSR C）。適合度や予測精度は高くはなかったが（$R^2Y=0.396$, $Q^2=0.118$）、S-lineプロット（@fig-feOPLSR D）の解析からは、aspartateが産後後期の便において相対的に高い寄与を示す代謝物として同定された。
// aspartate: 2.83ppm
...
2'-FLおよび3-FLはともにlactoseにfucoseが結合した構造を持つが、前者は$alpha$1,2-結合、後者は$alpha$1,3-結合と、その結合様式が異なる。FUT2 は 2'-FL の合成を担う一方で、3-FL の合成には関与せず、後者の合成には$alpha$1,3-フコシルトランスフェラーゼ（FUT3）が関与する。
両酵素はともにlactoseおよびfucoseの供与体であるGDP-fucoseを用いることから、2'-FL と 3-FL の合成は競合的な関係にあると推察される。

Se-群において 3-FL が高値を示した要因として、FUT2 の機能低下により 2'-FL の合成が抑制された結果、基質が 3-FL 合成経路へとシフトしたことが考えられる。
_Bifidobacterium_属による利用において、2′-FL と 3-FL は同一の輸送体および代謝経路を共有することが報告されていることから@jamesMetabolismPredominantHuman2019a 、Se-群における 3-FL の増加は、2′-FL の欠乏を機能的に補完している可能性が示唆される。ただし、細菌種によってはいずれかのオリゴ糖を選択的に利用する場合もあるため、今後は腸内細菌叢データとの照合による検証が必要である。
...
本コホートに参加した母親に対して行った調査によると、Se+群および Se-群の双方において、四分の一から半数の母親が授乳時に粉ミルクを使用していたため （@fig-nutrition）、Se+群の乳児において粉ミルク由来の lactulose が高頻度に検出されたことになる。
...
= 結論
本研究では、「岩見沢市母子健康調査」に参加する母子から回収した母乳および母子便試料のNMRメタボローム解析を通じ、授乳期における代謝動態を包括的に明らかにした 。
...
また、母親の2'–FL分泌型/非分泌型の差異が母乳中のHMO組成を決定づけるだけでなく、離乳前の乳児便中の2'-FL、3-FL、lactulose等のメタボロームにも顕著な差をもたらすことを示し、腸内細菌叢に与える役割の差異を示唆した。
// limitation
...
#figure(
  grid(
    columns: 1,
    image("Figures/10016010mMd_BM00541_1.svg", width: 70%),
    image("Figures/10016010mMd_BM00541_2.svg", width: 70%),
    image("Figures/10016010mMd_BM00541_3.svg", width: 70%),
    box(width: 100%)
  ),
  caption: [
...
左：スコアプロット。各色の楕円は各群の95%信頼区間を示す。右：ローディングプロット。
...
//   ),
//   caption: [
//     アミノ酸およびその関連代謝物の経時変化による母乳中代謝物濃度の比較。
//   ]
// )<fig-bmAmino>
...
//   ),
//   caption: [
    // 脂肪酸およびその他代謝物の経時変化による母乳中代謝物濃度の比較。
//   ]
// )<fig-bmOther>
...
A: スコアプロット。Se+（青色）は2'-FL分泌型の母親、Se-（橙色）は2'-FL非分泌型の母親を示す。各色の楕円は各群の95%信頼区間を示す。B: ローディングプロット。図中点の色は、VIPの値により色分けされている。（R2Y: 0.678, Q2: 0.606）
...
#v(1em)
また、以下の代謝物についてピークからの同定に成功した（上図には注釈を付記していない）。
...
A, C: スコアプロット（A: 乳児、C: 母親）。各色の楕円は各群の95%信頼区間を示す。B, D: ローディングプロット。
...
A, C: スコアプロット（A: 乳児（$R^2Y=0.907$, $Q^2=0.866$）, C: 母親（$R^2Y=0.396$, $Q^2=0.118$））。各色の楕円は各群の95%信頼区間を示す。B, D: S-lineプロット（B: 乳児, D: 母親）。横軸は#super[1]H-NMRのケミカルシフトである。縦軸はビン積分値の変動量を示し、ピーク強度が大きいほど絶対値は大きくなる。色分けはグループ間スコアとの相関係数を示し、高いほど群分けに寄与することを示す。
...
#figure(
  ...
  caption: [
    母体の分泌型（Se+/Se-）が乳児便メタボロームに及ぼす影響に関する OPLS-DA 判別分析
  ]
)<fig-feOPLSDA>
A, C, E: スコアプロット（Se+ vs Se-）。A: 出産直後（$R^2Y=0.701$, $Q^2=0.332$）、C: 産後1ヶ月（$R^2Y=0.882$, $Q^2=0.37$）、E: 産後4-5ヶ月（$R^2Y=0.747$, $Q^2=-0.107$）。各色の楕円は各群の95%信頼区間を示す。
...
本研究を遂行するにあたり、終始熱心かつ丁寧なご指導を賜りました北海道大学大学院先端生命科学研究院 蛋白質科学研究室の相沢智康教授、新井達也助教、津田栄博士に深く感謝申し上げます。また、副査として論文をご精読いただき、学術的な観点から貴重なご助言を賜りました同研究院 自然免疫研究室の中村公則教授、光生物学研究室の菊川峰志准教授に心より御礼申し上げます。
