...
本論文では、北海道岩見沢市で実施中の前向き母子コホート研究「岩見沢市母子健康調査（SMILE Iwamizawa）」 @komatsuAssociationsMaternalDiet2023 に参加する母親とその乳児を対象とし、核磁気共鳴（NMR）を用いたメタボローム解析により、母乳および糞便中の代謝物動態を包括的に評価した。
...
== 統計解析
母乳の濃度データおよび母子糞便のスペクトルビニングデータを用い、多変量解析ソフト SIMCA 18（Sartorius, Umeå, Sweden）にて多変量解析を実施した。
...
すべてのサンプルにおいて、解析に十分なシグナル強度を有する明瞭な#super[1]H-NMRスペクトルを取得した（@fig-feSpec）。代謝物の同定は、スペクトル上のシグナルパターンの解析およびChenomx内データベースとの照合により行い、最終的に計76種類の代謝物を同定した。化合物の定量については、スペクトルのパターンが複雑であることから、@sec5.2 で達成されたような高い再現性をもつ定量解析を実施することが現時点では困難であった。

== 母子糞便中メタボロームの主成分分析
母子それぞれの糞便メタボロームの時期的な変遷を俯瞰的に評価するため、NMRスペクトルのビン積分値データを用いた主成分分析（PCA）を実施した（@fig-fePCA）。
