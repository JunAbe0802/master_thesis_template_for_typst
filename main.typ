...
= 概要
乳児期の環境が将来の健康に影響を与えるというDOHaD仮説において、腸内細菌叢はその鍵を握る因子である 。本研究では、母乳および母子糞便の代謝物プロファイルを包括的に解析することで、授乳期における栄養供給と代謝機能の発達過程を統合的に理解することを目的とした 。具体的には、岩見沢市母子健康調査（SMILE Iwamizawa）に参加する母子を対象に、核磁気共鳴（NMR）を用いたメタボローム解析を実施した 。
...
群間の統計学的比較には、Welch の t検定または Tukey-HSD 法を用いた。
...
= 考察
本研究では、前向き母子コホート研究「岩見沢市母子健康調査（SMILE Iwamizawa）」の参加者を対象とし、核磁気共鳴（NMR）を用いたメタボローム解析を実施することで、母乳および母子糞便における代謝物動態の包括的な評価を試みた。母乳の成分組成と母児の腸内代謝環境の変化を統合的に解析した本成果は、乳児期における栄養供給と代謝機能の発達過程を理解する上で重要な知見を提供するものである。

母乳メタボローム解析の結果、産後の経過に伴いその組成がダイナミックに変動することが明らかとなった。特にヒトミルクオリゴ糖（HMOs）である 3′-SL、6′-SL、および 2′-FL は、授乳期間の進行とともに顕著な減少を示した。この傾向は先行研究の報告とも一致しており、@kortesniemi1HNMRMetabolomics2022 @poulsenDynamicChangesHuman2022 乳児の成長段階に応じた免疫調節や、初期腸内細菌叢形成におけるプレバイオティクスとしての役割を反映していると考えられる。
...
_Bifidobacteria_属よる利用において、2′-FL と 3-FL は同一の輸送体および代謝経路を共有することが報告されていることから@jamesMetabolismPredominantHuman2019a 、Se-群における 3-FL の増加は、2′-FL の欠乏を機能的に補完している可能性が示唆される。ただし、細菌種によってはいずれかのオリゴ糖を選択的に利用する場合もあるため、今後は菌叢データとの照合による検証が必要である。

また、Se+群で Fucose が高値であった理由については、Se-群での生成制限や Se+群での分解亢進といったフィードバック機構の関与が想定され、これにはトランスクリプトーム解析等を用いた分子レベルでの追究が求められる。なお、フコース代謝に関連する成分以外に顕著な差異が見られなかった事実は、FUT2 遺伝的多型が母乳メタボローム全体に及ぼす影響は、主要な HMOs の組成変化に限定的であることを示している。
...
LactuloseはHMOsと同様にBifidobacterium属を始めとした腸内細菌に利用されることが知られている@rasoulimehrabaniLactuloseSelectivelyStimulates2025。
...
= 結論
本研究では、SMILE Iwamizawaコホートの母乳および母子糞便サンプルのNMRメタボローム解析を通じ、授乳期における代謝動態を包括的に明らかにした 。

母乳サンプルの解析からは、授乳期間の進行に伴い主要な代謝物構成がシフトすることを捉え、母乳組成の動的変化を解明した。
母子の糞便サンプルの解析からは、産後期間において、母体の糞便メタボロームが安定していたのに対し、乳児では顕著な動的変遷が確認され、産後早期の乳児糞便における乳糖および単糖類（グルコース、ガラクトース）の高値は、新生児期の未熟な消化吸収能を反映していることを示した。
また、母親のFUT2遺伝的多型（分泌型/非分泌型）が母乳中のHMO組成を決定づけるだけでなく、離乳前の乳児糞便中の代謝物プロファイルにも顕著な差（2'-FL、3-FL、Lactulose等の濃度差）をもたらすことを突き止めた 。
